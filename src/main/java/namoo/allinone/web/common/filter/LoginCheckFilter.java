...
public class LoginCheckFilter implements Filter {
	...
	private static final String[] whitelist = { "/", "/css/*", "/fonts/*", "/js/*", "/members/register",
			"/members/signup", "/members/signin", "/members/logout" };//--->로그인이 필요없는 리스트
		...
		try {
			...
			if (!isWhiteList(requestURI)) {
				...
				if(session.getAttribute("loginMember") == null) {
					...
					httpResponse.sendRedirect("/members/signup?redirect=" + requestURI);//로그인요청시 이 곳을 통과하므로 redirect를 걸어줌
					return;
				}
			}
